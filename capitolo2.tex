...
In caso di squilibro tra i due valori di $D$ ed $H$ si parla di rete \textit{steady}; questa situazione si verifica quando il dislivello tra i due valori è tale da condizionare pesantemente i tempi di calcolo del \textit{nonce} e quindi il superamento dei previsti $10$ minuti. Una causa potrebbe essere che per una inflessione del valore dei Bitcoin alcuni \textit{miner} hanno abbandonato l'attività e quindi difficoltà risulta essere sbilanciata fino al successivo aggiornamento. In questo caso infatti si avrà che la rete è in defict di potenza e la probabilità $P$ non sia allineata. Ad esempio per una difficoltà $D=6567843252517$ e un hashrate totale di $H=42003661622$ \textit{GH/s} la probabilità di trovare un blocco in $10$ minuti ($600$ secondi) è:
\begin{equation}
